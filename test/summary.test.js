...
test("renderSummary keeps the AI handoff compact and actionable", () => {
  ...
  assert.match(markdown, /next stabilization: freeze and review documented JSON root fields before 0\.2\.0/);
  assert.match(markdown, /contract review: pending-0\.2\.0-review \/ node bin\/aienvmp\.js schema --json \/ discover, start, discovery, status, context/);
